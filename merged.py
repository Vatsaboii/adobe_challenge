...
def detect_curves(image):
    # Create a blank mask for detected curves
    mask = np.zeros(image.shape[:2], dtype=np.uint8)
    
    # Create copies of the original image
    image_with_annotations = image.copy()
    image_without_annotations = np.zeros_like(image)  # This will hold the image without annotations
    ...
    for contour in contours:
        ...
        if len(approx) > 5:
            if len(contour) >= 5:
                ...
                # Annotate the image with the ellipse
                cv2.ellipse(image_with_annotations, ellipse, (0, 255, 0), 2)
                cv2.putText(image_with_annotations, '', (int(ellipse[0][0]), int(ellipse[0][1])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        
        if len(approx) == 4:
            x, y, w, h = cv2.boundingRect(contour)
            aspect_ratio = float(w) / h
            if 0.9 <= aspect_ratio <= 1.1:
                # Draw the square on the mask
                cv2.drawContours(mask, [approx], -1, 255, 1)
                # Annotate the image with the square
                cv2.drawContours(image_with_annotations, [approx], -1, (0, 255, 0), 2)
                cv2.putText(image_with_annotations, '', (approx[0][0][0], approx[0][0][1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            else:
                # Draw the rectangle on the mask
                cv2.drawContours(mask, [approx], -1, 255, 1)
                # Annotate the image with the rectangle
                cv2.drawContours(image_with_annotations, [approx], -1, (255, 0, 0), 2)
                cv2.putText(image_with_annotations, '', (approx[0][0][0], approx[0][0][1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
        
        elif len(approx) > 4:
            # Draw the curve on the mask
            cv2.drawContours(mask, [approx], -1, 255, 1)
            # Annotate the image with the curve
            cv2.drawContours(image_with_annotations, [approx], -1, (255, 255, 0), 2)
            cv2.putText(image_with_annotations, '', (approx[0][0][0], approx[0][0][1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)

    # Apply the mask to the original image to create an image without annotations
    image_without_annotations = cv2.bitwise_and(image, image, mask=mask)

    return image_with_annotations, image_without_annotations
...
def main(image_path):
    ...
    image_with_curves, image_without_annotations = detect_curves(image.copy())
    
    # Save images
    cv2.imwrite('image_with_curves.jpg', image_with_curves)
    cv2.imwrite('image_without_annotations.jpg', image_without_annotations)
    
    # Display image with curves
...
if __name__ == "__main__":
    image_path = '/Users/srivatsapalepu/Downloads/problems/occlusion2.jpg'
    main(image_path)
